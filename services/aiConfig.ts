...
export const formatCalendarEventsForAI = (events: CalendarEvent[]): string => {
    ...
    const formatEvent = (event: CalendarEvent) => {
        ...
        if (!start) return `- "${summary}" (date inconnue)`;
        ...
        if (isAllDay) {
            return `- "${summary}" (toute la journée le ${startDate.toLocaleDateString('fr-FR', dateOptions)})`;
        }
        ...
        if (startDate.toDateString() === endDate.toDateString()) {
             return `- "${summary}" le ${dateString} de ${startTimeString} à ${endTimeString}`;
        } else {
            // Event spans multiple days
            const endDateString = endDate.toLocaleDateString('fr-FR', dateOptions);
            return `- "${summary}" du ${dateString} à ${startTimeString} au ${endDateString} à ${endTimeString}`;
        }
    };
...
const gmailTools: FunctionDeclaration[] = [
    {
        ...
        description: "Recherche dans la boîte de réception Gmail de l'utilisateur. Affiche les résultats et te prépare à lire un e-mail spécifique.",
        parameters: {
            type: Type.OBJECT,
            properties: {
    ...
    {
        name: 'lireEmail',
        description: "Lit le contenu complet d'un e-mail spécifique, généralement après une recherche. L'e-mail est également affiché à l'écran.",
        parameters: {
            type: Type.OBJECT,
            properties: {
                identifiantEmail: { type: Type.STRING, description: "Un identifiant pour trouver l'e-mail, comme sa position ('premier', 'deuxième'), son expéditeur ('celui de Jane'), ou son sujet ('celui sur le rapport')." },
            },
